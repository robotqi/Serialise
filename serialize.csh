...
# --------------------------------------
   ...
   o TEXT TEXT_WIDTH 10 "Text Width (Mils) :"
   o DTEXT TEXT_WIDTH $TEXT_WIDTH
   o OPTION T_A  "Text Angle :"
   o 0
   o 90
   o 180
   o 270
...
switch ($T_A)
	...
	breaksw        
endsw
set TEXT_WIDTH = `echo "$TEXT_WIDTH * 0.083333" | bc`
...
foreach step ($gREPEATstep)
	...
	if ( $angle  < 0 ) then
	    set angle = `echo "$angle + 360" | bc`
	endif
	
	set angle = `echo "$angle + $TEXT_ANGLE" | bc`
	set angle = `echo "$angle % 360 " | bc`	
